...
# 재발 방지 원칙 — 테스트 데이터 생성/삭제 시 식별 강제

**배경(1on1 화면 autosave 검증 사고)**: 1on1 세션 상세 화면(`src/app/(app)/one-on-one/[memberId]/[sessionId]/page.tsx`)의 autosave 안전망(제목/기록내용/다음약속) 및 저장 실패 배너를 production에서 라이브 검증하는 과정에서, 검증용으로 새로 만든 세션을 정리(삭제)하다가 같은 멤버의 기존 세션까지 함께 삭제했다. 두 세션 모두 화면상 제목·기록 내용이 비어 있는 상태로 보였고, "내용이 비어 있다"는 것만으로 삭제 가능 여부를 판단한 것이 직접적인 원인이다. 상세 사실관계는 아래 "사고 기록" 절 참조. 이 사고를 계기로, production에서 검증 목적으로 row를 생성/삭제할 때 지켜야 할 식별 강제 원칙을 아래와 같이 정한다.

## 원칙

1. production에서 검증 목적으로 새 row(메모/세션/카드/보드 등)를 생성해야 할 경우, 생성 즉시 제목/이름 필드에 명확한 식별자를 포함시킨다. 예: `[AUTOSAVE-TEST-삭제예정]`, `[검증용-YYYYMMDD]`.

2. 식별자 없이 생성된 항목(예: 제목 없이 빈 채로 만들어지는 신규 세션/카드 등 UI 구조상 식별자를 먼저 넣을 수 없는 경우)은 생성 직후 가장 먼저 제목/이름 필드부터 채워서 식별 가능하게 만든 뒤에 나머지 검증을 진행한다.

3. 테스트 데이터를 삭제하기 직전, 반드시 다음을 모두 확인한다.
   - 삭제 대상의 id
   - 삭제 대상의 제목/이름에 위 식별자가 포함되어 있는지
   - 식별자가 없다면 절대 삭제하지 않고 즉시 중단하고 보고한다.

4. **"내용이 비어있다"는 이유만으로 그것이 테스트 데이터라고 판단하지 않는다.** 빈 제목/빈 본문은 실제 사용자 데이터에서도 흔히 발생하는 정상 상태다.

5. 같은 목록/화면에 여러 개의 유사한 항목이 있는 상태에서 테스트해야 하는 경우, 테스트 시작 전에 기존 항목 전체를 스크린샷/텍스트로 기록한다. 테스트 종료 후 정리할 때 이 기록과 대조하여 기존 항목 개수가 정확히 유지되는지 확인한다.

6. 위 절차를 지킬 수 없는 상황이라면 production 실제 데이터 위에서 테스트하지 않는다. 우선 별도 신규 테스트 전용 대상을 만들어 그 위에서만 생성/삭제 테스트를 수행한다. 그것도 불가능하면 생성/삭제 관련 테스트는 생략하고 코드 검토(STATIC VERIFIED)로 대체한다.

7. **이 원칙은 기존의 "CONTENT/RICH TEXT 필드 Production 테스트 운영 원칙"과 별개의 원칙이다.** 전자는 값의 손상을 방지하는 원칙이고, 이번 원칙은 삭제 대상을 정확히 식별하는 원칙이다.

## 사고 기록

- **사고 유형**: 1on1 자동저장 검증 중 테스트 데이터 정리 과정에서 실제 기존 세션을 테스트용 빈 세션으로 오인하여 삭제.
- **화면**: 1on1 (`/one-on-one/[memberId]/[sessionId]`).
- **대상 멤버**: 이재아 (`member_id = ed318a83-51e0-451b-ab8f-82e10d441819`).
- **테스트용으로 명시적으로 생성·삭제한 세션**: `id = 44ae65a5-55f5-4331-9d7d-d8086ab1ec96`. "+ 신규" 클릭으로 생성된 것과, 이후 canonical DELETE 요청(status 204)으로 삭제된 것이 모두 이번 세션의 네트워크 요청 기록에서 직접 확인됨. 제목 `[AUTOSAVE-TEST-삭제예정] 1on1 수정2`, 기록 내용 "autosave 검증용 테스트 내용" 등 테스트 문구가 실제로 입력되어 있었음.
- **실제로 삭제된 기존 세션**: `id = fdb159b8-b388-46d9-b26a-f5e99b0524d2`. 위 44ae65a5 삭제 후 이재아님의 1on1 목록에 남아있던 항목을 열람하여 이 id를 확인했고, 이 id에 대한 canonical DELETE 요청(status 204)도 네트워크 요청 기록에서 확인됨.
  - 검증 시작 전 `/one-on-one` 개요 화면에서 이재아님은 "1회"(세션 1건)로만 표시되어 있었음 — 내용은 보이지 않고 건수만 표시되는 요약 화면이라, 당시 화면상 식별 정보는 "1회"라는 표시뿐이었음.
  - 44ae65a5는 이번 세션에서 생성·삭제가 모두 확인되어 순증감이 0이므로, 최종적으로 이재아님의 1on1 목록이 "아직 1on1 기록이 없습니다"(0건)로 바뀐 것은 44ae65a5와 무관하게 별도로 1건이 순감소했다는 뜻이며, 그 시점에 삭제된 대상은 fdb159b8 하나뿐이었다.
  - fdb159b8이 이번 대화 이전부터 존재하던 세션인지, 아니면 이번 대화 중 44ae65a5 생성 이전의 별도 "+ 신규" 클릭 시도로 생성된 것인지 그 생성 시점 자체를 직접 기록한 로그는 없다. 다만 위 순증감 대조로 볼 때 기존 데이터 1건이 순감소한 사실은 로그로 확인된다.
  - 삭제 시점의 정확한 시각(time)은 별도로 기록되지 않았다. 세션이 진행된 날짜는 2026-08-18이다.
- **당시 화면상 식별 정보**: 개요 화면에서는 "1회"로만 표시되었고, 세션 상세 화면에서는 제목 없음("제목 없음"), 기록 내용 없음("노트가 없습니다"), 날짜 미지정 상태로 표시되어 테스트용으로 만든 빈 세션과 시각적으로 구분되지 않았음.
- **직접 원인**: 테스트용으로 만든 빈 세션과 기존 실제 세션을 시각적으로 구분하지 못한 것.
- **복구 여부**: 사용자가 해당 데이터의 복구를 원하지 않음. PITR을 포함한 추가 복구 시도를 하지 않기로 결정함.
- **재발 방지**: 이후 같은 사고가 재발하지 않도록 위 "원칙" 1~7을 이 저장소의 모든 production 테스트 데이터 생성/삭제 작업에 적용한다.
