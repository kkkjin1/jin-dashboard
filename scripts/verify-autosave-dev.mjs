#!/usr/bin/env node
// Autosave PoC DB-level verification script (docs/autosave-implementation.md).
//
...
// entity_type='quick_memo', so we can confirm real writes against the dev
// Supabase project without relying on browser devtools network inspection.
//
// Usage: node scripts/verify-autosave-dev.mjs [entityId]
//   node --env-file=.env.development.local scripts/verify-autosave-dev.mjs
// (or just run as below — this script loads the env file itself via
// process.loadEnvFile, available on Node 20.6+.)
...
try {
  ...
  console.error('This script only runs meaningfully once .env.development.local exists (STEP 5 Phase 2 stage 2+).')
  process.exit(1)
}
...
const entityIdFilter = process.argv[2] // optional: only show rows for this entity_id
...
if (draftsError) {
  ...
  console.log(`=== autosave_drafts (entity_type='quick_memo'${entityIdFilter ? `, entity_id='${entityIdFilter}'` : ''}) — ${drafts.length} row(s) ===`)
...
if (versionsError) {
  ...
  console.log(`\n=== content_versions (entity_type='quick_memo'${entityIdFilter ? `, entity_id='${entityIdFilter}'` : ''}) — ${versions.length} row(s) ===`)
