...
export default function SettingsPage() {
  ...
  async function addMember() {
    ...
    const { data } = await supabase.from('members')
      .insert({ name: newName.trim(), part: newPart })
      .select().single()
  ...
  const corePart = members.filter(m => m.part === '코어')
  const bizPart = members.filter(m => m.part === '비즈')
  ...
  return (
    <div className="p-8 max-w-lg">
      ...
      {/* 팀원 목록 */}
      {[{ label: '코어파트', list: corePart }, { label: '비즈파트', list: bizPart }].map(({ label, list }) => (
        <div key={label} className="mb-6">
          <h2 className="text-sm font-semibold text-gray-500 mb-3">{label} ({list.length}명)</h2>
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            {list.length === 0 ? (
              <p className="text-sm text-gray-300 text-center py-4">팀원이 없습니다</p>
            ) : (
              list.map((member, idx) => (
                <div key={member.id} className={`flex items-center gap-3 px-4 py-3 ${idx !== 0 ? 'border-t border-gray-50' : ''} group`}>
                  {editingId === member.id ? (
                    <>
                      ...
                      <select
                        value={editPart}
                        onChange={e => setEditPart(e.target.value as Part)}
                    ...
                    <>
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => { setEditingId(member.id); setEditName(member.name); setEditPart(member.part) }}
