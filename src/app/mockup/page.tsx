      ...
      <div className="max-w-7xl mx-auto px-6 py-5 h-full flex flex-col gap-4">
        ...
        <div className="flex-shrink-0 flex items-center gap-4">
          ...
          <div className="ml-auto flex items-center gap-2 w-52 h-9 bg-white border border-slate-200 rounded-full px-3 shadow-sm">
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="업무 검색"
              className="flex-1 text-xs text-slate-700 bg-transparent focus:outline-none placeholder:text-slate-300" />
            {query && <button onClick={() => setQuery('')} className="text-slate-300 hover:text-slate-500 text-xs">×</button>}
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 grid grid-cols-12 gap-5">
          ...
          {/* ① 오늘 — col-span-5 */}
          <div className="col-span-5 bg-[#0F1E36] rounded-3xl flex flex-col overflow-hidden min-h-0">
            {/* 고정: 헤더 */}
              ...
              <div className="flex flex-col gap-2">
                {(filteredSchedule.length ? filteredSchedule : SCHEDULE).map((item, i) => (
                  <div key={i} className={`flex items-center gap-3 rounded-2xl px-4 py-3 ${
                    q && (item.title.includes(q) || item.tag.includes(q))
                    ...
                    <span className="text-white/30 text-[10px] border border-white/15 px-2 py-0.5 rounded-lg flex-shrink-0">{item.tag}</span>
                  </div>
                ))}
            ...
            {/* 고정: 메모 */}
            <div className="flex-shrink-0 px-7 pb-7 pt-0 border-t border-white/10">
              <p className="text-white/30 text-[10px] uppercase tracking-widest mt-5 mb-2">오늘의 메모</p>
          ...
          <div className="col-span-7 grid grid-cols-2 gap-5 min-h-0">
            ...
            {/* ② 내일 */}
            <div className="bg-white rounded-3xl flex flex-col overflow-hidden min-h-0 shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {[
                      ...
                      <div key={i} className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2">
                        ...
                        <span className="text-slate-400 text-[9px] border border-slate-200 px-1.5 py-0.5 rounded-lg flex-shrink-0">{item.tag}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {['JD 초안 검토', '면접 평가표 수정', '온보딩 일정 확인'].map((t, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-2 border border-slate-100 rounded-xl">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {[
                      ...
                      <div key={i} className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${item.done ? 'bg-slate-50 border-slate-100' : 'bg-white border-slate-100'}`}>
                        ...
                        <span className="text-[9px] font-semibold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded flex-shrink-0">{item.label}</span>
                        <span className={`text-xs flex-1 truncate ${item.done ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{item.text}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {filteredTodos.length === 0
                  ...
                  : filteredTodos.map((item, i) => (
                    <div key={i} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all ${
                      q && (item.text.includes(q) || item.tag.includes(q))
                        ? 'bg-blue-50 border border-blue-100'
                        : item.done ? 'bg-slate-50' : 'border border-slate-100'
                    }`}>
                      ...
                      <span className={`text-xs flex-1 min-w-0 truncate ${item.done ? 'text-slate-400 line-through' : 'text-slate-700 font-medium'}`}>{item.text}</span>
                      <span className="text-[9px] text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-full flex-shrink-0">{item.tag}</span>
                    </div>
                  ))
                }
              </div>
