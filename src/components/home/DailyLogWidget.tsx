      ...
      {isToday && (
        <div className="flex-shrink-0 flex items-center justify-between mt-3 pt-2.5 border-t border-gray-100">
          ...
          <button
            onClick={handleSend}
            disabled={!text.trim()}
            className="text-[11px] font-medium bg-[#4C7FE0] text-white px-3 py-1.5 rounded-full hover:bg-[#3A6CC8] disabled:opacity-40 disabled:cursor-not-allowed transition-all">
            → 회고로 전달
          </button>
        </div>
      )}
