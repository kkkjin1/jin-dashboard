  ...
  return (
    <div className="h-full flex flex-col p-4 font-sans">
      <div className="flex items-center gap-2 mb-3 flex-shrink-0">
        ...
        {totalAdded > 0 && !saving && (
          <span className="text-[9px] text-[#0F1E36]/50">{totalAdded}개 추가됨</span>
        )}
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide space-y-1">
        ...
        {manualItems.map((item, i) => (
          <div key={`manual-${i}`} className="flex items-start gap-2 px-2 py-1.5">
            <span className="w-1 h-1 rounded-full flex-shrink-0 mt-1.5 bg-[#0F1E36]/30" />
            <span className="flex-1 text-[11px] leading-relaxed text-gray-400 line-through">{item}</span>
            <span className="flex-shrink-0 text-[9px] text-gray-300">✓</span>
          </div>
        ))}

        {/* 회고 참조 줄 */}
        {!hasJournal && manualItems.length === 0 ? (
            ...
            {refLines.map((line, i) => {
              ...
              return (
                <button
                  ...
                  <span className={`flex-1 text-[11px] leading-relaxed ${isDone ? 'text-gray-300 line-through' : 'text-gray-600'}`}>
                    {line}
                  </span>
                  {!isDone && (
      ...
      <div className="flex-shrink-0 flex gap-1.5 mt-2.5 pt-2.5 border-t border-gray-100">
        <input
          ...
          disabled={saving}
          className="flex-1 text-[11px] border border-white/60 bg-white/50 rounded-lg px-3 py-1.5 focus:outline-none focus:border-gray-300 focus:bg-white/70 transition-all disabled:opacity-50"
        />
        <button
          onClick={handleManualAdd}
          disabled={saving || !newText.trim()}
          className="text-[11px] text-gray-400 hover:text-gray-600 border border-gray-200 rounded-lg px-2.5 py-1.5 hover:bg-white/50 transition-colors disabled:opacity-40">
          +
        </button>
      </div>
    </div>
