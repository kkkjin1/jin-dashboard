...
export default function QuickMemoPanel() {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    await supabase.from('quick_memos').insert({
      title: title.trim(),
  ...
  return (
    <>
      ...
      {/* 슬라이드업 패널 */}
      <div
        className={`fixed bottom-0 right-6 z-50 w-96 bg-white rounded-t-2xl shadow-2xl border border-gray-200 transition-transform duration-300 ${
      ...
      >
        <div className="p-5">
            ...
            {TAGS.map(t => (
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                  tag === t ? TAG_COLORS[t] : 'bg-gray-50 text-gray-400 border-gray-200 hover:bg-gray-100'
                }`}
              >
                {t}
              </button>
          ...
          <div className="flex justify-between items-center mt-3">
            <span className="text-xs text-gray-300">ESC로 닫기</span>
            <button
              onClick={handleSave}
