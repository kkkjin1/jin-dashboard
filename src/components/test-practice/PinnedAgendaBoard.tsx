...
export default function PinnedAgendaBoard({ items, selectedId, onSelect }: Props) {
  ...
  return (
    <div className="sticky top-0 z-10 -mx-4 md:-mx-6 px-4 md:px-6 pb-3" style={{ background: 'var(--bg-page)' }}>
      <button onClick={toggle} className="w-full flex items-center justify-between py-3">
        <span className="flex items-center gap-2.5 text-[15px] font-extrabold" style={{ color: isEmpty ? 'rgba(var(--text-rgb),0.55)' : 'rgba(var(--text-rgb),1)' }}>
          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md" style={{ background: 'rgba(239,68,68,0.14)', color: '#F87171' }}>P1</span>
          지금 집중할 과제
          <span className="font-medium text-[12px]" style={{ color: 'rgba(var(--text-rgb),0.4)' }}>· {items.length}개</span>
        </span>
        <span className="text-[11px]" style={{ color: 'rgba(var(--text-rgb),0.4)' }}>{collapsed ? '펼치기 ▼' : '접기 ▲'}</span>
      </button>
